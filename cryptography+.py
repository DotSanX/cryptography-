...
def create_and_store_password():
  ...
  while True:
    ...
    if is_password_strong(created_password):
      ...
      logging.warning('Попытка создания слабого пароля слишком простый')
      print("Пароль слишком простой. Используйте комбинацию цифр, заглавных и строчных букв.")

def user_interface():
  """ Интерфейс пользователя для работы с менеджером паролей"""
  print(" Добро пожаловать в менеджер паролей!")

  if not os.paht.exists(PASSWORD_FILE) or not os.path.exists(KEY_FILE):
  ...
  while True:
    ...
    choice = input("Введите '1' для шифрования, '2' для расшифровки, '0' для выхода: ")
    if choice == '1':
      data_to_encrypt = input("Введите данные для шифрования: ")
      encrypted_data = encrypted_data(data_to_encrypt, key)
      print(f"Зашифрованный данные: {encrypted_data}")
    elif choice == '2':
      decrypted_data = input("Введите данные для расшифрования: ")
